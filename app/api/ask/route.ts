...
export async function POST(request: Request) {
  ...
  if (!aiEnabled()) return NextResponse.json({ error: "ai_off" }, { status: 503 });
  ...
  try {
    const [{ data: profile }, { data: goals }, { data: projections }] = await Promise.all([
      supabase.from("profiles").select("display_name, pay_cycle, take_home_cents, essentials_cents, lifestyle_cents, buffer_cents, savings_cents, debt_cents, debt_rate_bps, started_on").eq("user_id", user.id).maybeSingle(),
      supabase.from("goals").select("id, kind, title, target_cents, target_date, priority, goal_type, status, why").eq("user_id", user.id).order("target_date"),
      supabase.from("goal_projections").select("goal_id, completion_month, required_monthly_cents, capacity_monthly_cents, achievable, alt_later_months, alt_smaller_target_cents, alt_extra_monthly_cents").eq("user_id", user.id),
    ]);
    ...
    if (res.stop_reason === "refusal") return NextResponse.json({ error: "refused" }, { status: 502 });
    const text = res.content.filter((b) => b.type === "text").map((b) => (b as { text: string }).text).join("\n").trim();
    if (!text || findBannedTerms(text).length > 0) {
      return NextResponse.json({ answer: "I can explain your path, but not recommend products. Every number on your roadmap comes from your own figures — open a milestone to read its why." });
    }
    return NextResponse.json({ answer: text, disclaimer: DISCLAIMER });
  } catch (err) {
